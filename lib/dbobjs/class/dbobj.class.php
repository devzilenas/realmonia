...
class Dbobj {
	...
	public function validateNotEmpty($name) {
		if($field = self::field($name)) {
			if($field->istext() && '' === trim($this->$name)) { 
	 ...
	 * @param mixed $what (optional) What to load.
	 * @return self|FALSE
	 */
	public static function load_by($where, $what = NULL) {
	...
	public function asTime($field) {
		return self::toTime($this->astm($field));
	}
	...
	public function to_s() {
		...
		if(empty($str)) {
			if(isset($this->id)) {
				$str = ucfirst(c2u(get_called_class(),' '))."#".$this->id;
			}
		}
		return $str;
	}
	...
	public function asbc($field_name) {
		$amount = new Monia(ConfigMonia::base_currency(), $this->$field_name);
	}
	...
	public function asm($field_name, Currency $currency = NULL) {
		if(NULL == $currency) {
			$currency = ConfigMonia::base_currency();
		}
	 ...
	 * @return string
	 */
	public static function format_money(Monia $amount, $decimals = 2, $dec_point = ',', $thousands_sep = ' ', $show_sign = FALSE) {
		/** format in groups */ 
		$sign = $show_sign && $amount->as_f() > 0 ? '+' : '' ;
		return $sign.number_format($amount->as_f(), $decimals, $dec_point, $thousands_sep).' '.$amount->currency()->name();
	}
	...
	public function __call($name, $args) {
		$ret = NULL;
		$cl  = u2c($name);
		if(class_exists($cl) && 'Dbobj' == get_parent_class($cl)) {
			/** person->hive() */
			$idfield  = $name."_id";
			if($field = self::field($idfield)) {
				$id  = $this->{$name."_id"};
				if($o = $cl::load($id)) {
					$ret = $o;
				}
				...
				/** user->person() */
				$idfield = c2u(get_called_class())."_id";
				if($field = $cl::field($idfield)) {
					if($o = $cl::load_by(array("$cl.$idfield" => $this->id))) {
						$ret = $o;
					}
				}
			}
	 ...
	 * @return Monia|NULL
	 */
	public static function total_on($name, SqlFilter $filter) {
		$ret = array();
		$nf = clone $filter;
		$nf->setWhat(sprintf('SUM(%s) as total', Dbobj::iq($name)));
		$nf->setJoin(NULL);
		$nf->setOn(NULL);
		$nf->setGroupBy(NULL);
		if($res = current(static::find($nf))) {
			$ret = new Monia(ConfigMonia::base_currency(), $res->total);
		}
		return $ret;
	}
	/**
	 * Total grouped.
	 * @param string $amount Field name.
	 * @param string $group_by Field name.
	 * @return array
	 */
	public static function total_grouped_by($group_by, $amount, SqlFilter $filter) {
		$ret = array();
		$filter->setWhat(join(',', array(sprintf('SUM(%s) as total', $amount), $filter->what)));
		$filter->setGroupBy($group_by);
		if($res = static::find($filter)) {
			$ret = $res;
		}
		return $ret;
	}
	...
	public function asDate($name) {
		return self::toDate($this->astm($name));
	}
